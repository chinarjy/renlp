...
from RENlp_Sentiment.utils.utils import make_seed, load_pkl
from RENlp_Sentiment.utils.process import process
from RENlp_Sentiment.utils.dataset import CustomDataset, collate_fn
import RENlp_Sentiment.models as models
from RENlp_Sentiment.utils.trainer import train, validate
from RENlp_Sentiment.logger import logger
from RENlp_Sentiment.utils.data_processer import load_file
...
if config.type == 'car':
    ...
    load_file('data/origin/sentiment36k.csv', 'csv')
elif config.type == 'eat':
    data_path = config.data_path1
    out_path = config.out_path1
    load_file('data/origin1/dataset1.txt', 'txt')
...
if not os.path.exists(out_path):  # 如果没有保存好的预处理数据
...
for epoch in range(1, config.epoch + 1):
    ...
    if macro_f1 > best_macro_f1:
        ...
        macro_model_name = model.save(name=config.type, epoch=epoch)
    if micro_f1 > best_micro_f1:
        best_micro_f1 = micro_f1
        best_micro_epoch = epoch
        best_micro_model = model
        micro_model_name = model.save(name=config.type, epoch=epoch)
