...
class NerProcessor(object):
    ...
    def get_examples_text(self, inputs: list):
        examples = []
        guid = 0
        for i, sen in enumerate(inputs):
            guid = i
            text = ''
            labels = ''
            for char in sen:
...
def get_Dataset(args, processor, tokenizer, mode="train", text=None):
    if mode == "train":
        ...
        examples = processor.get_examples_text(inputs=text)
    else:
