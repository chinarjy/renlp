...
from RENlp_NER.data_prcessor import *
from RENlp_NER.config import config
from RENlp_NER.logger import logger
from RENlp_NER.models.Bert_NER import Bert_NER
...
def main(config):
    ...
    # Prepare model
    bert_config = BertConfig.from_pretrained(args.bert_model+'/config.json', num_labels=num_labels, finetuning_task=args.task_name)
    model = Bert_NER.from_pretrained(args.bert_model+'/pytorch_model.bin', from_tf=False, config=bert_config)
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer, warmup_steps=warmup_steps, t_total=num_train_optimization_steps)
    if args.fp16:
        try:
    ...
    if args.do_train:
        ...
        label_map = {i: label for i, label in enumerate(label_list, 1)}
        model_config = {"bert_model": 'data/output/pytorch_model.bin', "do_lower": args.do_lower_case,
        ...
        model = Bert_NER.from_pretrained(args.output_dir)
        tokenizer = BertTokenizer.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)

    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        if args.eval_on == "dev":
            eval_examples = processor.get_examples(args.data_dir, 'dev')
        elif args.eval_on == "test":
            eval_examples = processor.get_examples(args.data_dir, 'test')
        else:
        ...
        y_pred = []
        label_map = {i: label for i, label in enumerate(label_list, 1)}
        label_id_dict = {v: k for k, v in label_map.items()}
        for input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask in tqdm(eval_dataloader,
            ...
            for i, label in enumerate(label_ids):
                ...
                for j, m in enumerate(label):
                    if j == 0:
                        continue
                    elif label_ids[i][j] == label_id_dict['[SEP]']:
